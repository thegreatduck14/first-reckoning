...
export type UnitCategory = "infantry" | "vehicle" | "siege" | "air" | "economy";
...
export interface UnitType {
  ...
  // Counter relationships: multiplier applied to this unit's attack
  // when attacking a target of the given category.
  attackBonusVs: Partial<Record<UnitCategory, number>>;
  // Flat defense ignored on the target when attacking a given category
  // (armor penetration) — more reliable than a multiplier against a
  // high, fixed defense stat.
  armorPenetrationVs?: Partial<Record<UnitCategory, number>>;
  // A resource cost outside the generic production-point system (e.g.
  // Colonization-style "tools"). Not yet spent by anything — Phase 0 has
  // no economy — but recorded now so the design intent isn't lost.
  toolsCost?: number;
  notes?: string;
}
